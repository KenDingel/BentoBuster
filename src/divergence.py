...
def calc_divergence_with_vwap(df, velo, range_input, is_long, div_candles, vwap_range):
    ...
    vwap_condition = (df['close'] > df['vwap'] and df['close'].shift(div_candles) < df['vwap'].shift(div_candles)) if is_long else \
                     (df['close'] < df['vwap'] and df['close'].shift(div_candles) > df['vwap'].shift(div_candles))
